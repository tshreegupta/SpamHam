...
def all_files(mypath):
    ...
    for dirname in listdir(mypath):
        ...
        labels.append(ltemp)
        part.append(ftemp)
    label_file=open("label_2a.txt","a")
...
pre_file=open("prediction_2c.txt","a")
...
for test in range(len(filenames)):  # 10 fold cross validation 
    ...
    # strings.
    train_data_features = vectorizer1.fit_transform(train_data)
    test_data_features = vectorizer1.transform(test_data)
    train_data_features = train_data_features.toarray()
    test_data_features = test_data_features.toarray()

    lda.fit(train_data_features,train_result)

    pre=lda.predict(test_data_features)
    pre=np.array(pre)
...
pre_file.write('prediction for 2b')
pre_file.write('\n')
pre_file.write(str(prediction))
pre_file.write('\n')
pre_file.write(str(prediction.sum()/10))
pre_file.write('\n')
print("prediction saved in prediction_2c.txt")
pre_file.close()
