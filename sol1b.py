...
def all_files(mypath):
    ...
    for dirname in listdir(mypath):
        ...
        labels.append(ltemp)
        part.append(ftemp)
    label_file=open("label_b.txt","a")
    label_file.write(str(datetime.now()))
    label_file.write('\n')
    label_file.write(str(labels))
    label_file.write('\n')
    print("labels saved in label_b.txt")
    label_file.close()
    return part, labels
        ...
        for y in range(len(part[x])):
            ...
            for word in rawdata.split():
                if i==0:                       #to remove first letter i.e. Subject
                    i=i+1
                elif not word.isdigit():       #to remove all numbers
                    fresh_data.append(word)
...
pre_file=open("prediction_b.txt","a")
...
pre_file.write('\n')
print("prediction saved in prediction_b.txt")
pre_file.close()